...
cpdef np.ndarray[np.complex128_t] TwistFFT(np.ndarray[np.int32_t] a,np.ndarray[np.complex128_t] twist):
    cdef int Ns2 = len(twist)
    cdef np.ndarray[np.double_t] b = np.double(a)
    return np.fft.fft(np.multiply((b[:Ns2]+1j*b[Ns2:2*Ns2]),twist))

cpdef np.ndarray[np.uint32_t] TwistIFFT(np.ndarray[np.complex128_t] a,np.ndarray[np.complex128_t] twist):
    cdef np.ndarray[np.complex128_t] b = np.multiply(np.fft.ifft(a),np.conjugate(twist))
    return np.append(np.real(b),np.imag(b))

cpdef np.ndarray[np.uint32_t] PolyMul(np.ndarray[np.uint32_t,ndim=1] a,np.ndarray[np.uint32_t,ndim=1] b,np.ndarray[np.complex128_t] twist): #a or b in R and other in T
