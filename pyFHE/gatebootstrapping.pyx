...
cdef inline PolynomialMulByXai(np.ndarray[np.uint32_t] poly,int a,int N):
    cdef int aa
    cdef int i
    if(a==0 or a == 2*N):
        return poly
...
cdef inline np.ndarray[np.uint32_t,ndim = 2] BlindRotateFFT(np.ndarray[np.complex128_t,ndim = 4] bkfft,np.ndarray[np.uint32_t] tlwe,np.ndarray[np.uint32_t,ndim = 2] trlwe,params):#t is TLWE and r is TRLWE polynomial to rotate and t is TLWE
