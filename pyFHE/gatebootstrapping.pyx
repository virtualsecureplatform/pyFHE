...
cdef inline np.ndarray[np.uint32_t,ndim = 2] BlindRotateFFT(np.ndarray[np.complex128_t,ndim = 4] bkfft,np.ndarray[np.uint32_t] tlwe,np.ndarray[np.uint32_t,ndim = 2] trlwe,params,fft,ifft):#t is TLWE and r is TRLWE polynomial to rotate and t is TLWE
