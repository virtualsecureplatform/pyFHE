...
def IdentityKeySwitch(tlwe, ck: CloudKey):
    ...
    return np.uint32(np.append(np.zeros(ck.params.n), tlwe[-1])) - np.sum(
        [
            np.sum(
                [
                    ck.ksk[i][j][
                        aibar[i] >> (ck.params.basebit * (ck.params.t - (j + 1))) & mask
                    ]
                    for j in range(ck.params.t)
                ],
                axis=0,
            )
            for i in range(ck.params.N)
        ],
        axis=0,
    )
