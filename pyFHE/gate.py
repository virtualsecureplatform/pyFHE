...
def HomMUX(a,b,c,ck):
    return IdentityKeySwitch(np.append(np.zeros(ck.params.n),2**29) + GateBootstrappingTLWE2TLWEFFT(np.append(np.zeros(ck.params.n),np.uint32(-2**29)) + a + b,ck) + GateBootstrappingTLWE2TLWEFFT(np.append(np.zeros(ck.params.n),np.uint32(-2**29)) - a + c,ck),ck)